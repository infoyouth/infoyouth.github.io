...
Let's dive into how Go handles conditional logic! Go uses `if`, `if-else`, and `if-else-if` statements to control the flow of your program based on conditions. The best part? No parentheses needed around your conditions, but braces `{}` are _mandatory_.

## <span style="color:#2980b9">`if` Statement 🤔</span>

The `if` statement executes a block of code _only_ if a condition is `true`.
...
Go provides a neat feature within `if` statements: the _short statement_. This allows you to declare and initialize a variable _within_ the `if` condition itself.
...
In this example, `x := getValue()` is executed _before_ the condition `x > 0` is evaluated. The variable `x` is only scoped to the `if` and `else` blocks.

- **Key Takeaways:**
  - Go _requires_ braces `{}`.
  - Go _doesn't need_ parentheses around conditions `()`.
...
- **Type Switch:** Used to determine the _type_ of a variable. Super handy when dealing with `interface{}`.
...
Normally, Go executes only the matching `case` and then _automatically_ exits the `switch`. The `fallthrough` keyword makes execution _continue_ to the next `case`, regardless of whether it matches.
...
Go _implicitly_ adds a `break` statement after each `case`. You _don't_ need to write `break` yourself. This helps prevent accidental fall-through bugs common in other languages.
...
Go keeps things simple! Instead of multiple loop types like `while` or `do-while` in other languages, Go rocks just _one_: the `for` loop. But don't let that fool you; it's super flexible and can handle all your looping needs.
...
The `range` keyword in Go is your friendly iterator! It makes looping through data structures _easy peasy_. Think of it as a helper that automatically gives you both the index and the value of each item.
...
- **Strings:**
  ...
  _Important Note:_ `range` on strings iterates over _runes_, which are Unicode code points (like characters).
...
- **`break`:** Think of `break` as an emergency exit. It immediately _terminates_ the loop's execution, even if the loop condition is still true.
...
- **`continue`:** `continue` is like a "fast forward" button. It _skips_ the current iteration and jumps to the next one.
