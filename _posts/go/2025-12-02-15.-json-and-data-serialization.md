...
description: "🚀 Master the art of JSON data serialization! This comprehensive guide dives into encoding, decoding, struct tags, handling unknown JSON, and streaming to empower your data exchange capabilities. ✨"
author: infoyouth
...
## <span style="color:#43e97b">🎯 Hands-On Assignment</span>

<details>
<summary><strong>💡 Project: Configuration Manager</strong> (Click to expand)</summary>
<br>
<p><strong>🚀 Your Challenge:</strong></p>
<p>Build a <strong>Configuration Manager</strong> that handles multiple data formats (JSON, YAML, TOML) and provides a unified interface for reading and writing configuration files. Your program should support format conversion and validation. ⚙️🔄</p>

<p><strong>📋 Requirements:</strong></p>
<p>Create a program with the following features:</p>

<p><strong>1. Config struct with these fields:</strong></p>
<ul>
<li><code>AppName</code> (string)</li>
<li><code>Version</code> (string)</li>
<li><code>Server</code> (nested struct with <code>Host</code>, <code>Port</code>, <code>TLS</code> bool)</li>
<li><code>Database</code> (nested struct with <code>Driver</code>, <code>Host</code>, <code>Port</code>, <code>Name</code>)</li>
<li><code>Features</code> (map[string]bool for feature flags)</li>
<li><code>MaxConnections</code> (int)</li>
</ul>

<p><strong>2. Core functions:</strong></p>
<ul>
<li><code>LoadConfig(filepath string) (*Config, error)</code> - auto-detect format</li>
<li><code>SaveConfig(config *Config, filepath string, format string) error</code></li>
<li><code>ConvertFormat(inputPath, outputPath, format string) error</code></li>
<li><code>ValidateConfig(config *Config) error</code> - check required fields</li>
</ul>

<p><strong>3. Format support:</strong> JSON, YAML, TOML</p>

<p><strong>💡 Implementation Hints:</strong></p>
<ul>
<li>Use struct tags for all three formats: <code>`json:"..." yaml:"..." toml:"..."`</code> 🏷️</li>
<li>Detect format by file extension (<code>.json</code>, <code>.yaml</code>, <code>.toml</code>)</li>
<li>Use <code>encoding/json</code>, <code>gopkg.in/yaml.v3</code>, <code>github.com/BurntSushi/toml</code></li>
<li>Implement custom validation rules (e.g., port range 1-65535)</li>
<li>Handle nested structs properly in all formats</li>
<li>Add helpful error messages for parsing failures</li>
</ul>
...
<p><strong>Converted to YAML (config.yaml):</strong></p>
<pre>
app_name: MyApp
version: 1.0.0
server:
...
<ul>
<li>Add environment variable override support (e.g., <code>APP_SERVER_PORT=9000</code>) 🔧</li>
<li>Implement config file watching with automatic reload</li>
<li>Add encryption/decryption for sensitive fields (passwords, API keys)</li>
<li>Create a diff function to compare two config files</li>
<li>Support partial config updates (merge instead of replace)</li>
<li>Add a CLI interface with commands: <code>load</code>, <code>save</code>, <code>convert</code>, <code>validate</code></li>
</ul>

<p><strong>Submission Guidelines:</strong></p>
<ul>
<li>Create sample config files in all three formats</li>
<li>Test format conversion in both directions</li>
<li>Include validation test cases (valid and invalid configs)</li>
<li>Share your complete code in the comments</li>
<li>Explain how you handled format differences</li>
<li>Show output from running your program</li>
</ul>

<p><strong>Share Your Solution! 💬</strong></p>
<p>Can't wait to see your implementations! <strong>Post your solution below</strong> and learn from other approaches. 🎨</p>
