...
```mermaid
graph LR
A[Your Program] --> B{Class.forName("MyClass")};
B --> C[Class Object];
C --> D(getDeclaredFields());
C --> E(getDeclaredMethods());
C --> F(getConstructors());
D --> G[Field Objects];
E --> H[Method Objects];
F --> I[Constructor Objects];
```
