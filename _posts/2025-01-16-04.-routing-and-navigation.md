...
C --> E[Module Loaded & Rendered];
D --> F[Continue with Base App];
E --> G[User Interaction Continues];
F --> G;
```
...
```mermaid
graph LR
A[User requests route] --> B{AuthGuard (CanActivate)};
B -- True --> C[Route Access Granted];
...
<a routerLink="/products/123">View Product 123</a>  <!--parameterized route-->
```
