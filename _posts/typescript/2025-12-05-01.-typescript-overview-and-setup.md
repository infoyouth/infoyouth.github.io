...
<details>
<summary><strong>💡 Project: User Greeter Application - Master TypeScript Type Safety</strong> (Click to expand)</summary>
<br>
<p><strong>🚀 Your Challenge:</strong></p>
<p>Create a comprehensive <strong>User Greeter Application</strong> that demonstrates TypeScript's type safety, interfaces, strict mode, and proper configuration. Your system should validate user input, handle optional properties, and showcase TypeScript's compile-time error prevention. 👥✨</p>

<p><strong>📋 Requirements:</strong></p>

<p><strong>Part 1: Project Setup & Configuration</strong></p>
<ul>
<li>Initialize a new TypeScript project:
  <ul>
    <li>Run <code>npm init -y</code> to create package.json</li>
    <li>Install TypeScript: <code>npm install --save-dev typescript</code></li>
    <li>Generate tsconfig.json: <code>npx tsc --init</code></li>
  </ul>
</li>
<li>Configure tsconfig.json:
  <ul>
    <li>Set <code>"outDir": "./dist"</code> for compiled JavaScript output</li>
    <li>Set <code>"rootDir": "./src"</code> for TypeScript source files</li>
    <li>Enable <code>"strict": true</code> for maximum type safety</li>
    <li>Optional: Enable <code>"noUnusedLocals"</code> and <code>"noUnusedParameters"</code></li>
  </ul>
</li>
<li>Create project structure: <code>src/</code> and <code>dist/</code> folders</li>
</ul>

<p><strong>Part 2: User Interface Design</strong></p>
<ul>
<li>Create <code>src/user.ts</code> and define a <code>User</code> interface:</li>
<li>Required properties:
  <ul>
    <li><code>name: string</code> - User's full name</li>
    <li><code>age: number</code> - User's age (must be positive)</li>
    <li><code>email: string</code> - User's email address</li>
  </ul>
</li>
<li>Optional properties:
  <ul>
    <li><code>country?: string</code> - User's country (optional)</li>
  </ul>
</li>
<li>Export the interface for use in other modules</li>
</ul>

<p><strong>Part 3: Function Implementation with Type Safety</strong></p>
<ul>
<li>Implement <code>createUser(name: string, age: number, email: string): User</code>
  <ul>
    <li>Validate age is positive (throw error if not)</li>
    <li>Validate email contains '@' symbol</li>
    <li>Return properly typed User object</li>
  </ul>
</li>
<li>Implement <code>printUser(user: User): void</code>
  <ul>
    <li>Display formatted greeting with user details</li>
    <li>Handle optional country property safely</li>
    <li>Use optional chaining <code>user.country?.</code> for safe access</li>
  </ul>
</li>
<li>Add proper type annotations to all function parameters and return types</li>
</ul>

<p><strong>Part 4: Main Application Logic</strong></p>
<ul>
<li>Create <code>src/index.ts</code> as the entry point</li>
<li>Import User interface and functions from user.ts</li>
<li>Create 2-3 sample users with different properties</li>
<li>Call printUser for each created user</li>
<li>Demonstrate type safety by including commented-out invalid code</li>
</ul>

<p><strong>Part 5: Compilation & Execution</strong></p>
<ul>
<li>Compile TypeScript: <code>npx tsc</code></li>
<li>Verify JavaScript output in <code>dist/</code> folder</li>
<li>Run compiled code: <code>node dist/index.js</code></li>
<li>Observe type checking during compilation</li>
</ul>

<p><strong>💡 Implementation Hints:</strong></p>
<ul>
<li><strong>Step 1:</strong> Start with interface definition: <code>interface User { ... }</code></li>
<li><strong>Step 2:</strong> Use type guards for validation: <code>if (age <= 0) throw new Error(...)</code></li>
<li><strong>Step 3:</strong> Enable strict mode to catch null/undefined issues early</li>
<li><strong>Step 4:</strong> Use optional chaining: <code>user.country?.toUpperCase()</code></li>
<li><strong>Step 5:</strong> Export/import syntax: <code>export interface User</code> and <code>import { User } from './user'</code></li>
<li><strong>Step 6:</strong> Try invalid code (e.g., passing string to age) to see TypeScript errors</li>
<li><strong>Bonus:</strong> Use template literals for formatted output: <code>`Hello, ${user.name}!`</code></li>
</ul>
...
export function printUser(user: User): void {
  console.log(`Hello, ${user.name} (${user.age})! Welcome aboard!`);
...
// src/index.ts
import { User, createUser, printUser } from './user';

const alice = createUser('Alice', 25, 'alice@example.com');
const bob = createUser('Bob', 30, 'bob@example.com');
...
<ul>
<li><strong>Challenge 1:</strong> Add a <code>Role</code> enum (Admin, User, Guest) and include it in User interface</li>
<li><strong>Challenge 2:</strong> Implement type guards: <code>function isAdmin(user: User): user is Admin { ... }</code></li>
<li><strong>Challenge 3:</strong> Create <code>printDetailedUser()</code> that handles all optional properties</li>
<li><strong>Challenge 4:</strong> Set up Jest for unit testing with <code>ts-jest</code></li>
<li><strong>Challenge 5:</strong> Build a <code>UserRepository</code> class with CRUD operations</li>
<li><strong>Challenge 6:</strong> Use generics: <code>class Greeter<T extends User> { ... }</code></li>
</ul>

<p><strong>Share Your Solution! 💬</strong></p>
<p>Built your User Greeter? Awesome! Share your implementation in the comments below. What type-safety benefits did you discover? Did TypeScript catch any bugs before runtime? Let's discuss! 🚀</p>
