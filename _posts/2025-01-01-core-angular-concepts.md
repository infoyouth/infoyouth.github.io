...
export class GreetingComponent implements OnChanges{
    ...
    ngOnChanges(changes : SimpleChanges): void {
        if(changes['name']) {
            console.log('The name changed from ', changes['name'].previousValue, ' to ', changes['name'].currentValue);
        }
    }
}
```
...
@Component({
  selector: 'app-child',
  template: `<p>Hello, <b>{{ userName }}</b> !</p>`
})
...
@Component({
  selector: 'app-parent',
  template: `
    <app-counter [counter]="initialCount" (counterChange)="updateCount($event)"></app-counter>
...
```html
<p>Formatted and Uppercased Date: {{ myDate | date: 'shortDate' | uppercase }}</p>
```
