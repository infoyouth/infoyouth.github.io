...
{% include quiz.html 
  question="Which Python library is commonly used as an ORM for database interactions?" 
  options="Pandas, SQLAlchemy, NumPy, Matplotlib" 
  correct="1" 
  explanation="SQLAlchemy is the most popular SQL toolkit and Object-Relational Mapping (ORM) library for Python." 
%}

{% include quiz.html 
  question="What object is used to execute SQL commands in Python's sqlite3 module?" 
  options="Connection, Cursor, Executor, Runner" 
  correct="1" 
  explanation="A cursor object is created from the connection and is used to execute SQL commands and fetch results." 
%}

{% include quiz.html 
  question="In the context of database transactions, what does the 'A' in ACID stand for?" 
  options="Accuracy, Availability, Atomicity, Authorization" 
  correct="2" 
  explanation="Atomicity ensures that a transaction is treated as a single unit, which either succeeds completely or fails completely." 
%}
...
{% include quiz.html 
  ...
  correct="2" 
  explanation="MongoDB stores data in BSON (Binary JSON), which allows for flexible and complex data structures." 
%}

{% include quiz.html 
  question="What is the best practice to prevent SQL injection when using sqlite3?" 
  options="String concatenation, f-strings, Parameterized queries, Raw SQL strings" 
  correct="2" 
  explanation="Parameterized queries (using placeholders like ?) ensure that user input is treated as data, not executable code, preventing SQL injection." 
%}
