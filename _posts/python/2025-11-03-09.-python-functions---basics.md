...
{% include code-playground.html 
   language="python" 
   code="# Python Functions - Definition and Calling
...
# Example 3: Temperature converters
def celsius_to_fahrenheit(celsius):
...
# Example 4: Rectangle area calculator
def calculate_rectangle_area(length, width):
    area = length * width
    perimeter = 2 * (length + width)
    return area, perimeter

area, perimeter = calculate_rectangle_area(10, 5)
...
{% include code-playground.html 
   language="python" 
   code="# Function Parameters and Arguments
...
def build_profile(name, age, city, occupation):
    profile = f\"{name} is {age} years old, lives in {city}, and works as a {occupation}.\"
    return profile

# Using keyword arguments - order can vary
profile1 = build_profile(name=\"Alice\", age=30, city=\"New York\", occupation=\"Engineer\")
profile2 = build_profile(city=\"London\", occupation=\"Designer\", name=\"Bob\", age=25)
...
register_user(\"alice123\", \"alice@email.com\")

# Mixing positional and keyword
register_user(\"bob456\", \"bob@email.com\", age=25, country=\"Canada\")

print(\"=\"* 50 + \"\\n\")

# Example 4: Order processing with multiple arguments
def calculate_order_total(item_price, quantity, shipping=0, discount=0):
    subtotal = item_price * quantity
    total = subtotal + shipping - (subtotal * discount)
    return total

# Different ways to call the function
print(f\"Basic order: ${calculate_order_total(50, 2):.2f}\")
print(f\"With shipping: ${calculate_order_total(50, 2, shipping=10):.2f}\")
print(f\"With discount: ${calculate_order_total(50, 2, shipping=0, discount=0.10):.2f}\")
print(f\"Full options: ${calculate_order_total(item_price=50, quantity=2, shipping=10, discount=0.15):.2f}\")" 
   height="750px" 
   gradient="green" 
%}
...
{% include code-playground.html 
   language="python" 
   code="# Return Statement Examples
...
# Example 2: Multiple return values (using tuples)
def get_user_info(user_id):
    ...
    return users.get(user_id, (\"Unknown\", \"N/A\", 0))
...
person = create_person(\"Charlie\", 28, \"Paris\")
print(f\"Created person: {person}\")

print(\"\\n\" + \"=\"* 50 + \"\\n\")

# Example 6: Safe division with multiple outputs
...
# Test safe division
value, msg = divide_safe(10, 2)
print(f\"Result: {value}, Message: {msg}\")

value, msg = divide_safe(10, 0)
print(f\"Result: {value}, Message: {msg}\")" 
   height="850px" 
   gradient="pink" 
%}
...
# Overriding defaults
user2 = create_user(\"bob\", \"bob@email.com\", role=\"admin\", active=False)
print(f\"User 2: {user2}\")

print(\"\\n\" + \"=\"* 50 + \"\\n\")

# Example 2: Notification system with defaults
def send_notification(user, message, priority=\"normal\", email=True):
    \"\"\"
    Send a notification to a user.
    
    Args:
        user (str): Username to notify
    ...
    print(f\"Priority: {priority}\")
    print(f\"Email notification: {'Yes' if email else 'No'}\")
    print()

send_notification(\"Alice\", \"Your order shipped!\")
send_notification(\"Bob\", \"System maintenance tonight\", priority=\"high\")

print(\"=\"* 50 + \"\\n\")

# Example 3: Database query function with defaults
def query_database(table, limit=10, offset=0, order_by=\"id\"):
    \"\"\"
    Query a database table with pagination and sorting.
    
    Args:
        table (str): Name of the table to query
        limit (int): Maximum number of records to return
        offset (int): Number of records to skip
        order_by (str): Column name for sorting
    
    Returns:
        str: Simulated SQL query string
    \"\"\"
    query = f\"SELECT * FROM {table} ORDER BY {order_by} LIMIT {limit} OFFSET {offset}\"
    return query

# Different query configurations
print(query_database(\"users\"))
print(query_database(\"products\", limit=20))
print(query_database(\"orders\", limit=5, offset=10, order_by=\"created_at\"))

print(\"\\n\" + \"=\"* 50 + \"\\n\")

# Example 4: Using help() to view docstrings
...
<details>
<summary><strong>💡 Project: Personal Finance Calculator</strong> (Click to expand)</summary>
<br>
<p><strong>Your Challenge:</strong></p>
<p>Build a personal finance calculator that uses all function concepts (parameters, return values, default arguments, keyword arguments, and docstrings).</p>

<p><strong>Requirements:</strong></p>

<p><strong>Part 1: Income Calculator</strong></p>
<ul>
<li>Create <code>calculate_monthly_income(hourly_rate, hours_per_week)</code></li>
<li>Assume 4 weeks per month, return total monthly income</li>
</ul>

<p><strong>Part 2: Budget Tracker</strong></p>
<ul>
<li>Create <code>calculate_remaining_budget(income, rent, utilities, groceries, transport, entertainment)</code></li>
<li>Calculate total expenses and return remaining budget</li>
</ul>

<p><strong>Part 3: Savings Goal</strong></p>
<ul>
<li>Create <code>savings_plan(current_savings, monthly_contribution, goal_amount, interest_rate=0.02)</code></li>
<li>Return: months_needed, total_contributed, interest_earned, final_amount</li>
</ul>

<p><strong>Part 4: Tax Calculator</strong></p>
<ul>
<li>Create <code>calculate_taxes(income, tax_rate=0.20, deductions=0, credits=0)</code></li>
<li>Return: gross_tax, net_tax, take_home_pay</li>
</ul>

<p><strong>Part 5: Comprehensive Report</strong></p>
<ul>
<li>Create <code>generate_financial_report(**kwargs)</code> with docstring</li>
<li>Use all previous functions to display formatted report</li>
</ul>

<p><strong>Bonus Challenges:</strong></p>
<ul>
<li>Add input validation (check for negative values)</li>
<li>Create a debt payoff calculator function</li>
<li>Add emergency fund recommendation</li>
...
<p>Completed the project? <strong>Post your code in the comments below!</strong> Show us your implementation! 🎨</p>
