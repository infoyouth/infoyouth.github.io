...
```mermaid
graph LR
    A[Start] --> B{Some Operation};
    B -- Success --> C{Another Operation};
    C -- Success --> D{Yet Another Operation};
    D -- Success --> E[End];
...
## <span style="color:#2980b9">When Should You *Maybe* Use `goto`? 🤔</span>
...
```mermaid
flowchart TD
    A[Start File Operation] --> B{Check Return Value?};
    B -- Yes --> C{Error?};
    C -- Yes --> D{Check errno?};
    D -- Yes --> E[Handle Specific Error];
    D -- No --> F[Handle Generic Error];
...
graph LR
    A[Start] --> B{Denominator == 0?};
    B -- Yes --> C[Print Error Message 1];
    C --> D[Return DIVIDE_BY_ZERO_ERROR];
    B -- No --> E{Denominator < 0?};
    E -- Yes --> F[Print Error Message 2];
